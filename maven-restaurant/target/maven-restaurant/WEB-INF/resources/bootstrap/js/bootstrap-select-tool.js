(function ($) {
   ...
   $.fn.bootstrapSelect = function (options, param) {
       ...
       if (options.data) {
           ...
           //var param = {};
           options.onBeforeLoad.call(target, options.queryParams);
           if (!options.url) return;
           
           $.ajaxSettings.async = false; //设置getJson同步
           $.getJSON(options.url, options.queryParams, function (data) {
        	   console.log("in url");
        	   console.log(data);
               init(target, data);
           });
           $.ajaxSettings.async = true; //恢复getJson异步
       }
       ...
       target.on('changed.bs.select', options.onChange);
