...
(function($){
	...
	function wrapTabs(container) {
		var $tabNav = $(container).find(".nav-tabs");
		var $tabContent = $(container).find(".tab-content");
		if($tabNav.length == 0){
			$('<ul class="nav nav-tabs"></ul>').prependTo(container);
		}
		if($tabContent.length == 0){
			$('<ul class="tab-content"></ul>').prependTo(container);
		}
		return;
	...
	function bindEvents(container,tab){
//		tab.bind('contextmenu', function(e){
//				var state = $.data(container, 'tabs')
//				var opts = state.options;
//				opts.onContextMenu.call(container, e, tab.html(),0);
//		});
	}
	...
	function addTab(container, options) {
		...
		var $tabContent = $(container).find(".tab-content");
		var sortNum = (sortNum = $tabNav.children(":last-child").find("a").data("sort"))?sortNum+1:1;
		$('<li ><a href="#pane_'+options.title+'" id="'+options.title+'" data-ol="'+options.ol+'" data-toggle="tab" data-sort="'+sortNum+'" >'+options.title+'</a></li>').appendTo($tabNav);
		$('<div  class="tab-pane " id="pane_'+options.title+'"></div>').append(options.content).appendTo($tabContent);
		
		var newTab = $("#"+options.title);
		if (typeof(options.closable) == "undefined" || options.closable){ 
			newTab.addClass("tabs-closable");
		}
		newTab.on("click",function(){
			newTab.tab("show");
		})
		newTab.on("dblclick",function(){
			closeTab(container,options.title);
		})
		newTab.tab("show");
		if($(opts.contextMenu).length>0){
			newTab.contextmenu({
				'target1':opts.contextMenu,
				 onItem: function(context, e) {
				  	var tab = context.context;
					var t = $(e.target).text();
					if(t == "刷新"){
						$("#pane_"+tab.id).find("iframe").attr('src', $("#pane_"+tab.id).find("iframe").attr('src'));
					}else if(t == "关闭"){
						closeTab(container,tab.id);
					}else if(t == "收藏"){ 
						top.collection($("#pane_"+tab.id).find("iframe").attr('src'));
					}else if(t == "关闭其他标签"){
						var $tabs = $tabNav.find("a");  
				         for(var i = $tabs.length - 1; i >= 0; i--) {  
				            if(tab.id != $tabs[i].id) {  
				            	closeTab(container,$tabs[i].id);
				            }
				         } 
				         $(tab).tab("show");
					}else if(t == "关闭右侧标签"){
		...
		newTab.on("click",function(){
			if (options.onSelect){
				options.onSelect.call(this,options.title,sortNum);
			}
		});
		return;
	...
	function closeTab(container, which) {
		...
		if(tab.hasClass("tabs-closable")){
			tab.parent().prev().find("a").tab("show");
			$("#pane_"+tab.attr("id")).remove();
			tab.parent().remove();
		}
		return;
...
//		setScrollers(container);
	...
	function getSelectedFrame(container){
		return $(container).find(".tab-pane.active").find("iframe");
	}
	function getFrameById(container,which){
		return $(container).find("#pane_"+which).find("iframe");
	}
	...
	function exists(container, which){
		return getTab(container, which).length > 0;
	}
	...
	$.fn.tabs.methods = {
		...
		getSelectedFrame: function(jq){
			return getSelectedFrame(jq[0]);
		},
		getFrameById: function(jq,which){
			return getFrameById(jq[0],which);
		},
	...
	$.fn.tabs.parseOptions = function(target){
	  ...
	  $.fn.tab = function(action){
		  if(action != "show"){
			  _tabShow.call(this,action);
			  return;
		  }
		  top.setOL(this.data("ol"));
		  _tabShow.call(this,action);
	  }
