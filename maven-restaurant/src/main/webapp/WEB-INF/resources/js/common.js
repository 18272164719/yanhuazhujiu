...
(function($) {
	...
	$.redirectLogin = function(redirectUrl) {
		var loginUrl = "/shopxxb2b2c/business/login";
		if ($.trim(redirectUrl) != "") {
...
$().ready(function() {
	...
	var $refreshToggle = $(".refresh");
	var $exportXlsToggle = $(".exportXls");
	...
	var $dropdownMenu = $(".dropdown .dropdown-menu").find("li");
	...
	if ($.fn.fileinput != null) {
		...
		$fileinput.each(function() {
			...
			var $file = $('<input name="file" type="file">').insertAfter($element).fileinput({
				uploadUrl: "/shopxxb2b2c/business/file/upload",
				uploadExtraData: {
	...
	$refreshToggle.click(function() {
		var table = $(this).parents(".bootstrap-table1").find("table");
		table.bootstrapTable("refresh");
		return false;
	});
	// 导出xls
	$exportXlsToggle.click(function() {
		var table = $(this).parents(".bootstrap-table1").find("table");
		var fileName = $(this).data("filename")||"exportXls_medicshop";
		table.tableExport($.extend({}, {
	         type: "xls",
	         fileName:fileName,
	         escape: false
	     }));
	...
	$dropdownMenu.click(function() {
		var $dropdownToggle = $(this).parents(".dropdown").find("[data-toggle=dropdown]");
		$dropdownToggle.find("span").text($(this).find("a").text());
		var table = $(this).parents(".bootstrap-table1").find("table");
		var code = $(this).data("code");
		table.bootstrapTable("refreshOptions",{
	 		queryParams:function(params){
	 			params[$dropdownToggle.data("name")] = code;
				return params;
			}
		});
			...
			callbacks: {
				onImageUpload: function(files) {
					...
					$files.each(function() {
						...
						$.ajax({
							url: "/shopxxb2b2c/business/file/upload",
							type: "POST",
							data: formData,
							dataType: "json",
							contentType: false,
