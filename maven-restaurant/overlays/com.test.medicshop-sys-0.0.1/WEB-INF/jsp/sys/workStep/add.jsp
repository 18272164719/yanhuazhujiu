				...
				<div class="form-group">
					...
					<div  class="col-xs-7">
						<div class="input-group">
						  ...
						  <input class="form-control" id="workUrlUrl" name="workUrlUrl"  type="text" onclick=selectWorkUrl(this) />
						</div>
					</div>
...
function selectWorkUrl(obj){
	...
	$(function() {
		...
		//表单验证
		$('#form1').initValid(["name","group.id","workUrlUrl"]);
	});

	function  submitAjax(){
		if(!h.isValid($("#form1"))) return false;
		h.ajax({
			url :"${pageContext.request.contextPath}/sys/workStep/add.htmlx",
