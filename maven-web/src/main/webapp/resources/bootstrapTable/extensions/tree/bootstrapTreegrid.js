(function ($) {
    ...
    var getChild = function (node, source, field) {
        var data = [];
        var items = $.grep(source, function (item, index) {
            return item.parentId == node[field]||item.ParentId == node[field];
        });
    ...
    BootstrapTable.prototype.initData = function () {
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var roots = $.grep(this.data, function (row, index) {
            	...
            	return row.parentId == that.options.rootId ||row.Level == that.options.treeRootLevel;
            });
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        var child = $.grep(data, function (d, i) {
                            return (d.ParentId == item[that.options.treeId]||d.parentId == item[that.options.treeId]) && !d.hidden;
                        });
                        var child1 = $.grep(data, function (d, i) {
                            return (d.ParentId == item[that.options.treeId]||d.parentId == item[that.options.treeId]);
                        });
                        ...
                        icon = sprintf('<span class="tree-icon %s" style="cursor: pointer; margin: 0px 5px;min-width:12px;display:inline-block;"></span>', child.length > 0 ? that.options.expandIcon : (child1.length>0?that.options.collapseIcon:""));
                        ...
                        if(item.treePath){
                        	...
                        	if(path.substring(0,1)==","){
                  		   ...
                  		   lvl = path.split(',').length - 2;
                  	    } 
                        var indent = item.Level == that.options.treeRootLevel||item.parentId==that.options.rootId ? '' : sprintf('<span style="margin-left: %spx;"></span>', lvl * 20);
                        //var indent = item.Level == that.options.Level ||d.parentId==-1  ? '' : sprintf('<span style="margin-left: %spx;"></span>', 130);
                        
                       
                       var ic = item.icon;
                       ic = ic?  ic.replace("fa-fw",""):"";
                       if(ic=="" || ic.indexOf("fa")<0) {
                    	   if(child.length>0)
                    		   ic="fa fa-folder-open";
                    	   else {
                    		   if(child1.length>0)
                    			   ic="fa fa-folder";
                    		   else
                       ...
                       //value = '<i class="'+ic+' bg-aqua"></i> '+value;
                       value = '<i class="'+ic+' "></i> '+value;
                    }
        ...
        this.$body.find('> tr[data-index] > td > .tree-icon').on('click', function (e) {
            //e.stopPropagation();
            var $this = $(this),
                $tr = $this.parent().parent(),
                index = $tr.data('index'),
